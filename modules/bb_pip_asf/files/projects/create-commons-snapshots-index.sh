#!/bin/sh
cd /x1/buildmaster/master1/public_html/projects/xmlgraphics/commons/snapshots
...
if [ -f commons-$TODAY.jar ]; then
    echo '<tr>' >> ../table.inc.tmp
    find . -name "commons-$TODAY*" -exec echo '<td class="basic"><a href={} title={}>{} </a></td>' \; | sort -r | sed -e s/\\.\\///g >> ../table.inc.tmp
    echo '</tr>' >> ../table.inc.tmp
