...
class Options(usage.Options):
    optParameters = [
        ...
        ['project-paths','pp', None,
         "The project mapping file"],
        ['include', 'f', None,
         '''\
...
def split_file_dummy(changed_file, project_paths):
...
_PATTERNS = {}
def patterns(project_paths):
    "Memoizing wrapper around re.compile()ing project_paths."
    if project_paths not in _PATTERNS:
        _PATTERNS[project_paths] = tuple(
          re.compile(line.strip())
          for line in open(project_paths, 'r').readlines()
        )
    return _PATTERNS[project_paths]
...
def split_file_branches(changed_file, project_paths):
    for regexp in patterns(project_paths):
...
class ChangeSender:
    ...
    def getChanges(self, opts):
        ...
        changed = commands.getoutput('%s changed %s "%s"' % (
            SL, rev_arg, repo)).split('\n')
        # the first 4 columns can contain status information
        changed = [x[4:] for x in changed]

        message = commands.getoutput('%s log %s "%s"' % (SL, rev_arg, repo))
        who = commands.getoutput('%s author %s "%s"' % (SL, rev_arg, repo))
    ...
    """ % (fltpat, expat)
        ...
        files_per_branch = {}
        for f in changed:
             branch, filename = split_file(f, opts['project-paths'])
