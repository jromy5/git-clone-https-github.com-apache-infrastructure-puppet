#!/bin/sh
cd /x1/buildmaster/master1/public_html/projects/xmlgraphics/fop/snapshots
...
if [ -f fop-$TODAY.jar ]; then
    echo '<tr>' >> ../table.inc.tmp
    find . -name "fop-$TODAY*" -exec echo '<td class="basic"><a href={} title={}>{} </a></td>' \; | sort -r | sed -e s/\\.\\///g >> ../table.inc.tmp
    echo '</tr>' >> ../table.inc.tmp
