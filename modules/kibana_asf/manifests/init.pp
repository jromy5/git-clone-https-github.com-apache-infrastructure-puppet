...
class kibana_asf (
  ...
  exec { 'sed kibana config':
    command => '/bin/sed -e "32s/.*/elasticsearch: {server: \"https:\/\/\"+window.location.hostname+\"\/_query\", withCredentials: true},/" < /usr/local/etc/logproxy/kibana/config.js > /usr/local/etc/logproxy/kibana/config.js2 && /bin/mv /usr/local/etc/logproxy/kibana/config.js2 /usr/local/etc/logproxy/kibana/config.js', # lint:ignore:80chars
  }
