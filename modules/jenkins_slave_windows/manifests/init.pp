class jenkins_slave_windows (
  ...
  $java_jenkins.each |String $jdk| {
    ...
    exec { "extract ${jdk}" :
      command => "powershell.exe Expand-Archive -Force F:\\jenkins\\tools\\java\\asf-build-${jdk}.zip -DestinationPath F:\\jenkins\\tools\\java\\${jdk}",
  ...
  exec { "create symlink for JDK1.9":
    ...
    onlyif    => "if (Test-Path 'F:\\jenkins\\tools\\java\\latest1.9') { exit 0;}  else { exit 1; }",
    provider => powershell,
  }
    ...
    onlyif    => "if (Test-Path 'F:\\jenkins\\tools\\java\\latest1.8') { exit 0;}  else { exit 1; }",
    provider => powershell,
  }
}