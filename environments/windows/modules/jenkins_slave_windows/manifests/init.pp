...
################### create symlinks #############################
  ...
  exec { "create symlink for JDK1.9":
    ...
    command      => "powershell.exe cmd.exe /c mklink /d F:\\jenkins\\tools\\java\\latest1.8 F:\\jenkins\\tools\\java\\jdk1.8.0_152",
    onlyif    => "if (Test-Path 'F:\\jenkins\\tools\\java\\latest1.8') { exit 1;}  else { exit 0; }",
    provider => powershell,
  }
  exec { "create symlink for JDK1.7":
    command      => "powershell.exe cmd.exe /c mklink /d F:\\jenkins\\tools\\java\\latest1.7 F:\\jenkins\\tools\\java\\jdk1.7.0_79-unlimited-security",
    onlyif    => "if (Test-Path 'F:\\jenkins\\tools\\java\\latest1.7') { exit 1;}  else { exit 0; }",
    provider => powershell,
  }
  exec { "create symlink for JDK1.6":
    command      => "powershell.exe cmd.exe /c mklink /d F:\\jenkins\\tools\\java\\latest1.6 F:\\jenkins\\tools\\java\\jdk1.6.0_30",
    onlyif    => "if (Test-Path 'F:\\jenkins\\tools\\java\\latest1.6') { exit 1;}  else { exit 0; }",
    provider => powershell,
  }
  exec { "create symlink for JDK1.5":
    command      => "powershell.exe cmd.exe /c mklink /d F:\\jenkins\\tools\\java\\latest1.5 F:\\jenkins\\tools\\java\\jdk1.5.0_22-64",
