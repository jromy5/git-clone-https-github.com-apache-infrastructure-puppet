...
class kibana_asf {
  ...
  exec { 'download kibana 3':
    ...
    unless  => 'ls /tmp/ | grep -qs kibana.tgz',
    before  => Exec['untar kibana'],
  } ->
  ...
  exec { 'untar kibana':
    ...
    require => Exec['download kibana 3'],
    before  => Exec['sed kibana config'],
  } ->

  exec { 'sed kibana config':
    command => '/bin/sed "s/:9200\"/\/_query\", withCredentials: true}/" < /usr/local/etc/logproxy/kibana/config.js | /bin/sed "s/http/https/g" > /usr/local/etc/logproxy/kibana/kibana/config.js', # lint:ignore:80chars
